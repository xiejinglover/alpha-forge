...
description: 对大量候选量化策略执行可复现的 N 选优研究：先确认开发期与留出集，只用开发期评分进行重复随机小组锦标赛，再按胜出次数加权或唯一成员等权汇总调仓日 Top1 股票/ETF 投票，生成全标的或 Top-K 票数加权组合。用于用户要求实施、复现或审计 N 选优、随机分组选优、赢家投票组合及其留出回测时；不用于门槛过滤、去相关或重新开发回测框架。
---
...
- 稳定唯一的 `candidate_id`；
- 开发期评分或候选净收益；
- `rebalance_date, candidate_id, asset_id` 语义的 Top1 信号；
- 原仓库正式组合回测入口。
...
python3 scripts/run_n_select.py vote \
  ...
  --signals holdout_top1_signals.csv \
  --top-k 1,3,5 \
...
首版只处理每个成员在每个调仓日输出一个 Top1 标的的场景。不实现资格门槛、去相关、成员 Top-K、入选标的等权或替代回测框架。
