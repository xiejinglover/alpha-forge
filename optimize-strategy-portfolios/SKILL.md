...
执行前必须完整读取 [EMA20 标准流程](references/ema20-canonical-workflow.md)、[数据与实验契约](references/data-and-experiment-contract.md) 和 [内置风险因子收益](references/bundled-risk-data.md)。

## 1. 以目标仓库为事实来源

先定位候选策略的已扣费正式账户日收益、稳定 `candidate_id`、各家族自身 full 基准和已有质量诊断。控制回归默认使用 Skill 内置、经清单校验的九列日频因子收益；复用仓库现有加载和回测入口，只写字段适配器，不重训模型、不重生信号、不改交易执行。

若目标仓库已有 EMA20 同口径的 `eligible`、`robust_quality_score` 和风险回归产物，优先复现并直接消费。若没有，严格按标准流程重算，不得用原始 Sharpe、年化收益或临时综合分替代。

内置因子只覆盖 `2008-01-02` 至 `2026-08-31`。任何基准交易日超出覆盖范围时停止；不得补0、前向填充或静默缩短回归日历。用户明确提供替代因子收益时可通过 `--controls` 覆盖，但必须核对字段、日期、单位、来源和 PIT 边界，推荐同时传入完整性清单。
...
- 内置风险因子缺日、哈希不符或研究区间越过其截止日时停止；不得绕过完整性检查后继续给出控制后指标。
