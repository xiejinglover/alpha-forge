...
`assets/risk-data/ema20_control_factor_returns_asof_2026-08-31.csv`

首列为 `trade_date`，其余九列为因子模拟组合的日简单收益。该文件由相邻 manifest 固定行数、日期、字节数和 SHA-256；运行器默认校验后加载。它不是个股逐日风格暴露矩阵，不能直接用于持仓八维向量或风险簇聚类。

需要覆盖默认数据时，`--controls` 同时支持 `date,factor_id,return` 长表和 `trade_date,<factor columns>` 宽表。同日因子集合必须一致；任一基准交易日缺失时停止，不用0补齐、不前向填充、不取日期交集。外部文件推荐用 `--controls-manifest` 提供完整性与来源清单。

内置快照的字段含义、来源及 PIT 限制见 [内置风险因子收益](bundled-risk-data.md)。
