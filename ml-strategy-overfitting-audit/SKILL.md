...
description: 审计现有机器学习量化策略仓库的过拟合、时间隔离、因果合法性、随机种子稳定性、特征与容量消融、walk-forward 稳健性、股票池迁移、模型后选择、标签敏感性及特征扰动风险。用于检查已有训练、验证和回测代码；必须优先复用仓库现有入口执行实验，不重新开发策略或框架，并产出逐步骤、逐模块、可追溯的详细审计报告。
---
...
先完整读取 [审计执行与报告规范](references/audit-execution-and-report.md)，再执行任何实验。始终把用户提供的策略仓库视为事实真源。

1. 先检查仓库级指令、README、依赖清单、配置、训练入口、验证入口、回测入口、测试和历史产物。
2. 使用 `rg`、配置文件和调用关系定位真实代码路径，建立 `repository_execution_map`：记录入口命令、函数/类、参数来源、数据输入、输出产物和现有测试。
3. 先确认唯一审计对象：具体 ML 策略、生产训练配置/模型插件、权威基线 run 和 development/final 数据身份。仓库只有通用框架、没有具体 ML 策略，或存在多个候选时，列出已发现候选并询问用户，不得自行选择。
4. 先按仓库原始方式复现一个基线运行；不能复现时停止依赖该基线的实验，记录错误和影响。
5. 对每个 ML 模块分别记录执行方式和状态。执行方式只使用 `REUSE_DIRECT`、`REUSE_WITH_OVERRIDE`、`NO_SAFE_REUSE_PATH` 或 `NOT_APPLICABLE`；状态使用 `COMPLETED`、`PARTIAL`、`NEEDS_USER_INPUT`、`SKIPPED_UNAVAILABLE`、`NOT_APPLICABLE` 或 `FAILED_TO_RUN`。
6. 只允许复用现有训练、验证、回测和数据处理代码，通过已有配置、CLI 参数、函数参数或最小适配器改变实验变量。
7. 禁止新建替代性的训练器、模型管线、数据管线、组合构建器或回测框架；禁止复制策略核心逻辑到 Skill 脚本中重新实现。
8. 最小适配器只能调用已有单次实验入口、批量生成配置、收集产物或转换审计格式。如果多 seed、完整 OOF、噪声注入等实验需要适配器自行实现训练、切分、预测或数据变换语义，则使用 `NO_SAFE_REUSE_PATH` 并跳过。
9. 保持仓库原有数据口径、标签、股票池、费用、交易约束、fold 和随机源；某模块要求改变的变量除外，并逐项记录控制变量。
10. 使用隔离的实验输出目录，不覆盖基线模型、缓存、预测、账本或报告；记录代码提交、配置哈希、命令和产物路径。
11. 入口、参数含义、数据许可、计算预算或实验选择存在实质歧义时，先向用户提出一个聚焦问题；不要自行猜测。
12. 缺少现有入口、必要数据、依赖、硬件或授权，且完成实验将要求重新开发策略/框架时，跳过该模块并标记 `NO_SAFE_REUSE_PATH / SKIPPED_UNAVAILABLE`。列明检查过的证据、跳过原因、阻断类型、结论影响及恢复实验所需条件。
13. 不得把代码阅读、静态推断或脚本成功运行冒充实验通过；每个结论必须指向真实命令、产物和指标。文中的“历史依据”只说明规范来源，绝不能当作当前仓库的实验或通过证据。

## 强制报告产出

每次审计都必须产出完整 Markdown 报告，不能只返回三个顶层 verdict。报告必须包含：

- 审计对象、代码版本、环境、数据范围、策略与标签口径。
- 仓库理解结果和 `repository_execution_map`，列出实际复用的代码路径、符号与命令。
- 基线复现过程、输入、产物、指标、是否与仓库历史结果一致及差异解释。
- 从仓库发现到最终结论的逐步骤日志：每步写明目标、动作/命令、输入、输出、结果和阶段结论。
- 全部 10 个模块的独立章节；即使跳过或不适用也不得省略。
- 每个模块的假设、执行状态、复用代码、实验变量、控制变量、fold/seed/case、指标表、产物路径、事实、解释、模块 verdict、限制和下一步。
- OOS 访问账本、完整 trial 分母、`selection_record`、结构性违规和缺失事实。
- 汇总结论：已检验、未检验、失败、跳过、不适用和仍需用户确认的事项必须分开。
- 命令日志、生成/修改文件、错误与重试记录作为附录。

使用 [审计执行与报告规范](references/audit-execution-and-report.md) 中的字段和章节。允许写文件时，将结构化结果交给 `scripts/build_audit_report.py` 生成报告骨架，再补入图表、仓库特定解释和证据链接；只读或禁止创建文件时，直接在最终响应中按同一完整结构输出。报告中禁止使用没有对应证据的“通过”“稳健”或“无过拟合”表述。
...
脚本仅依赖 Python 标准库，既可直接执行，也可导入其中的函数。这些脚本不实现模型训练、case 重训或交易回测；Skill 应亲自调用被审计仓库的现有入口完成这些实验，再用本节脚本审计和汇总产物；不得把本应自动执行的实验交给用户手工完成。
...
- `scripts/build_audit_report.py`：验证逐步骤与逐模块结果是否齐全，并生成统一 Markdown 审计报告。

前三个审计脚本以 CSV 为输入、JSON 为输出；报告脚本以 JSON 为输入、Markdown 为输出。先运行 `python3 scripts/<script>.py --help` 查看字段定义；结构性错误必须修复，统计阈值则必须由研究方案预注册。
...
**脚本**：把每个合法标签编码为 `case_id`，用 `compute_stability_metrics.py` 生成成对 IC/RankIC、Top-K overlap、排序相关和预测漂移；Skill 再从被审计仓库已有回测账本中提取成交率、现金、成本和净收益。仓库未产出这些字段时记录缺失，不要要求用户手工补表。
