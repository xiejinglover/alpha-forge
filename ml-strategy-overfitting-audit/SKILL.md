...
description: 审计现有机器学习量化策略仓库的过拟合、时间隔离、因果合法性、随机种子稳定性、特征与容量消融、walk-forward 稳健性、股票池迁移、模型后选择、标签敏感性及特征扰动风险。用于检查已有训练、验证和回测代码；必须优先复用仓库现有入口执行实验，不重新开发策略或框架，并产出结论优先、总—分结构的可追溯审计报告。
---
...
每次审计都必须产出完整 Markdown 报告，但默认面向研究决策者，不把审计底稿直接堆进正文。使用“总—分”结构：

1. 先给总体结论：`eligibility_verdict`、`overfitting_verdict`、最重要发现、证据边界和优先行动。
2. 再给模块总览表：10 个模块的执行状态、verdict 和一句话结论。
3. 最后逐模块分析：结论置顶，然后写检验了什么、关键指标、事实与解释、证据边界和下一步。跳过或不适用的模块仍要保留。

正文只用简短的阶段表说明“做了什么、得到什么结论”。不在正文展开代码路径、函数名、shell 命令、哈希、原始 JSON、完整 case 列表、产物清单、OOS 账本或 `selection_record`。这些内容仍必须保留在结构化底稿中；只在用户要求技术证据或使用 `--include-technical-appendix` 时追加。

模块关键指标使用紧凑表格，不直接输出指标 JSON；过长指标集在正文中最多展示 12 条，其余保留底稿。正文叙述使用用户的语言，不直接照抄英文机器字段。禁止使用没有对应证据的“通过”“稳健”或“无过拟合”表述。完整字段和章节见 [审计执行与报告规范](references/audit-execution-and-report.md)。
...
- `scripts/build_audit_report.py`：验证逐步骤与逐模块结果是否齐全，默认生成结论优先的总—分报告；用户要求时才追加技术证据附录。
