...
报告必须按 `ML-001`、`ML-002`、`ML-003`、`ML-004`、`ML-005`、`ML-007`、`ML-008`、`ML-009`、`ML-010`、`ML-011` 顺序出现。每个模块包含：

### 身份与状态

- 模块名称、检验角色和优先级。
- `execution_mode`：`REUSE_DIRECT`、`REUSE_WITH_OVERRIDE`、`NO_SAFE_REUSE_PATH` 或 `NOT_APPLICABLE`。
- `execution_status`：`COMPLETED`、`PARTIAL`、`NEEDS_USER_INPUT`、`SKIPPED_UNAVAILABLE`、`NOT_APPLICABLE` 或 `FAILED_TO_RUN`。
- `blocked_reason_type`：未阻塞时为 `null`；阻塞时使用规定枚举。
...
- 仓库中已发现的相关实现与历史证据。
- 复用的代码路径、符号、配置和命令。
- 基线 case、实验 case、fold、seed、数据范围和样本量。
- 唯一改变的变量和保持不变的控制变量。
...
- 按实际顺序列出命令和运行状态。
- 列出模型、预测、指标、日志和账本产物的路径与哈希。
- 使用表格报告全部 case，不只报告赢家。
- 区分 train、development OOF/internal walk-forward 和 locked final OOS。
- 记录错误、重试、人工操作和偏离预注册方案的行为。
